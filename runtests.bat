set ShouldPause=1
set FBPath="E:\fastbuild\tmp\x64-Release\Tools\FBuild\FBuildApp\FBuild.exe"
set FBArgs="-dist -ide -clean"
cd %~dp0\externaltests\bees\solution
"%~dp0/msfastbuild/bin/Debug/msfastbuild.exe" -p bees.vcxproj -c Debug -f Win32 -a%FBArgs% -b%FBPath% - true
IF DEFINED ShouldPause pause
cd %~dp0\externaltests\SDL2-2.0.4\VisualC\SDL\
